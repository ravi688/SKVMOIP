...
public:
  ...
  m_videoIPAddress(videoIPAddress), m_keyMoIPAddress(keyMoIPAddress), m_videoPortNumber(videoPortNumber), m_keyMoPortNumber(keyMoPortNumber), m_videoUSBPortNumber(videoUSBPortNumber)
  {
    u32 len = strlen(name);
    _assert(len < 255);
...
static std::vector<MachineData> GetMachineDataListFromServer()
{
  ...
  {
    { IP_ADDRESS(192, 168, 1, 17), IP_ADDRESS(192, 168, 1, 18), 100, 101, "Dummy Machine", 1  },
